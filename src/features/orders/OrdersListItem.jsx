...
function OrdersListItem({ order }) {
  ...
  return (
    <li className="flex rounded-md bg-neutral-200">
      <div className="basis-1/4 xs:max-w-32 xs:basis-1/3 sm:max-w-40">
        <img
      ...
      <div className="flex grow flex-col px-4 py-3 xs:py-4 sm:px-6 sm:py-6">
        <div className="mb-1 flex items-center justify-between xs:mb-2">
          <div className="text-xs font-medium xs:text-sm sm:text-lg">
            {productName}
          </div>
          <div className="text-sm font-semibold text-lime-950 xs:text-base sm:text-xl">
            {formatCurrency(price)}
          </div>
        </div>
        <div className="mb-2 text-[10px] opacity-85 xs:mb-4 xs:text-xs sm:text-base">
          Quantity: {quantity}
        </div>
        <div className="mt-auto flex items-center justify-between text-[10px] xs:text-xs sm:text-base">
          <div>
